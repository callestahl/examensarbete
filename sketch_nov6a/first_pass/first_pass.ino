      ...
      if (SerialBT.available() >= 2) {
        ...
        if (table_index < osci.tables_capacity) {
          if (sample_index == 0) {
            ...
            osci.tables[table_index].samples = (uint16_t*)calloc(osci.samples_per_cycle, sizeof(uint16_t));
            if (osci.tables[table_index].samples != NULL) {
              osci.total_cycles++;
            } else {
              // NOTE(Linus): Logging
...
void generate_sine_wave() {
  for (int i = 0; i < WAVETABLE_SIZE; i++) {
    sine_wave[i] = (uint16_t)(MAX_16BIT_VALUE / 2 * (1 + sin(2 * M_PI * i / WAVETABLE_SIZE)));
  }
}
...
void wavetable_oscillation() {
  ...
  if (current_time >= next_sample_time) {
    uint16_t value = wave_table_linear_interpolation(osci.tables + selected_cycle, osci.samples_per_cycle, osci.phase); 
